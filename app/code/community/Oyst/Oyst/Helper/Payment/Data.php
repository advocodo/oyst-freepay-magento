...
class Oyst_Oyst_Helper_Payment_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function syncFromNotification($event, $data)
    {
        ...
        if (Oyst_Oyst_Model_Payment_Method_Freepay::EVENT_CODE_AUTHORISATION == $data['event_code'] && $data['success']) {
            $result = $this->invoice($order, $data);
        }

        if (Oyst_Oyst_Model_Payment_Method_Freepay::EVENT_CODE_CAPTURE == $data['event_code'] && $data['success']) {
            $result = $this->invoice($order, $data);
        }

        if (Oyst_Oyst_Model_Payment_Method_Freepay::EVENT_CODE_CANCELLATION == $data['event_code'] && $data['success']) {
            $result = $this->cancel($order, $paymentId);
        }

        if (Oyst_Oyst_Model_Payment_Method_Freepay::EVENT_CODE_REFUND == $data['event_code'] && $data['success']) {
    ...
    protected function _addTransaction($orderId, $transactionData)
    {
        ...
        if ($order->getId() && $paymentId) {
            ...
            $additionalInfo = array(
                $oystHelper->__('Payment Service Providers') => Oyst_Oyst_Model_Payment_Method_Freepay::PAYMENT_METHOD_NAME,
            ...
            if (!empty($transactionData['additional_data']) && isset($transactionData['additional_data']['expiry_date'])) {
                ...
                $additionalInfo[Mage::helper('oyst_oyst')->__('Expiration Date')] = $expiryMonth . ' / ' . $expiryYear;
            }
            ...
            if (Oyst_Oyst_Model_Payment_Method_Freepay::EVENT_CODE_AUTHORISATION == $transactionData['event_code']) {
                $payment->registerAuthorizationNotification($amount);
            } elseif (Oyst_Oyst_Model_Payment_Method_Freepay::EVENT_CODE_CAPTURE == $transactionData['event_code']) {
